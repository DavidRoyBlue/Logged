/**
 * oauth-notion — Supabase Edge Function
 *
 * Routes:
 *   GET /oauth-notion?user_id=<uid>       → redirect to Notion consent page
 *   GET /oauth-notion?state=<s>&code=<c>  → handle Notion callback (linking flow)
 */
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const result = await handleAuthorize("notion", userId, deps);
  if (result.redirect) {
    return Response.redirect(result.redirect, result.status);
  }
  return new Response(JSON.stringify(result.body ?? {}), {
